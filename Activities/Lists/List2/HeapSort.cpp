...
struct Person
{
    ...
    bool operator<(const Person& other) const
    {
        if(this->time < other.time)
            return true;
        if(this->time == other.time && this->name < other.name)
            return true;
        else
            return false;
    }
...
int main(void)
{
    size_t numFinishers; std::cin >> numFinishers;
    Person* arrFinishers = new Person[numFinishers]; 

    for(size_t i = 0; i < numFinishers; i++)
    {
        std::string name; size_t time; std::cin >> name >> time;
        arrFinishers[i] = {name, time};
    }

    Heap<Person>::sort(arrFinishers, numFinishers);
    
    for(size_t i = 0; i < numFinishers; i++)
        std::cout << arrFinishers[i].name << " " << arrFinishers[i].time << "\n";
