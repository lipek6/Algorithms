...
private:
    ...
    struct QuickSort
    {
        ...
        {
            static size_t PartitionR(T* array, size_t left, size_t right)
            {
            ...
            static void RecursivelyQuickSortR(T* array, size_t left, size_t right)
            {
                if(left >= right)
                    return;
        ...
        struct DescendingOrder
        {
            static size_t PartitionR(T* array, size_t left, size_t right)
            {
                ...
                while(true)
                {
                    while(array[++i] > pivot)           // Searching for element SMALLER than the pivot
                        if(i == right) break;               // Avoiding going out of bounds to the right
                    while(array[--j] < pivot)           // Searching for element GREATER than the pivot
                        if(j == left) break;                // Avoiding going out of bounds to the left
        ...
        static void QuickSortR(T* array, const size_t size, const bool ascendingOrder)
        {
            if(size <= 1)
                return;
         
            if(ascendingOrder == true)
                AscendingOrder::RecursivelyQuickSortR(array, 0, size - 1);
            else
                DescendingOrder::RecursivelyQuickSortR(array, 0, size - 1);
        }
    };
...
public:
    ...
    {
        QuickSort::QuickSortR(array, usedSize, ascendingOrder);
    }
